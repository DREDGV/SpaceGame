...
Опциональный массив верхнего уровня: сквозное описание **одной игровой системы** через все эпохи (дополняет краткие строки `eras[].systems.<systemId>` в матрице).
...
| `byEra` | object | Ключи **только** `A` … `K`; значения — непустые строки. Для записи с `systemId: "people"` валидатор требует **все** ключи A–K и непустые строки. Другие `systemId` — по мере введения можно ужесточать правила в валидаторе. |
