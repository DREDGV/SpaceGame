...
- **Новые фичи** без явного места в матрице эпох **A–G** (и без опоры в документах ниже) **не следует** включать в ближайшие спринты, пока рамка не согласована с костяком. Канонические названия эпох: `ERA_PROGRESSION_MAP.md` (раздел «Единая линейка эпох A–G»).

Документы рамки (корень репозитория):

- [`A_B_TO_C_BRIDGE.md`](../A_B_TO_C_BRIDGE.md) — мост от прототипа A–B к эпохе C
...
- `A_B_TO_C_BRIDGE.md` (repo root — A–B → C)
